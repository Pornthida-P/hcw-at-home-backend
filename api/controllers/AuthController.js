...
module.exports = {
    ...
    try {
      ...
      if (!invite) {
        sails.config.customLogger.log(
          "warn",
          "Login invite attempted with an invalid token.",
          null,
          "message",
          null,
        );
      ...
      passport.authenticate("invite", async (err, user) => {
        if (err || !user) {
          sails.config.customLogger.log(
            "warn",
            `Failed invitation authentication. ${err?.message || err}`,
            null,
            "server-action",
            null,
          );
        ...
        sails.config.customLogger.log(
          "info",
          `Invitation login: user ${user.id} authenticated successfully.`,
          null,
          "message",
          user.id,
        );
        ...
        try {
          await User.updateOne({ id: user.id }).set({
            lastLoginType: "invite",
          });
          sails.config.customLogger.log(
            "verbose",
            `User ${user.id} updated lastLoginType to "invite".`,
            null,
            "server-action",
            user.id,
          );
          ...
          sails.config.customLogger.log(
            "error",
            `Error updating user ${user.id} login type: ${updateError}`,
            null,
            "server-action",
            user.id,
          );
        ...
        req.logIn(user, function (err) {
          ...
          if (err) {
            sails.config.customLogger.log(
              "error",
              `Error during login for user ${user.id}: ${err}`,
              null,
              "server-action",
              user.id,
            );
          ...
          sails.config.customLogger.log(
            "info",
            `User ${user.id} logged in using invite. isExpert: ${isExpert}.`,
            null,
            "message",
            user.id,
          );
        ...
        sails.config.customLogger.log(
          "error",
          `Error in passport.authenticate callback for login invite: ${err}`,
          null,
          "server-action",
          null,
        );
      ...
      sails.config.customLogger.log(
        "error",
        `Unexpected error in loginInvite: ${e?.message || e}`,
        null,
        "server-action",
        null,
      );
  ...
  async forgotPassword(req, res) {
    try {
      try {
        ...
        sails.config.customLogger.log(
          "warn",
          "Invalid email provided for forgotPassword.",
          null,
          "message",
          null,
        );
      ...
      const appSecret =
        process.env.APP_SECRET || sails.config.globals.APP_SECRET;
      ...
      if (user) {
        try {
          await db
            .collection("user")
            .updateOne({ _id: user._id }, { $set: { resetPasswordToken } });
          sails.config.customLogger.log(
            "info",
            `User ${user._id} updated with resetPasswordToken.`,
            null,
            "server-action",
            user?._id,
          );
          ...
          sails.config.customLogger.log(
            "error",
            `Error updating user ${user._id} with resetPasswordToken: ${updateError.message}`,
            null,
            "server-action",
            user?._id,
          );
        ...
        try {
          ...
          sails.config.customLogger.log(
            "info",
            `Forgot password email sent to user ${user._id}.`,
            null,
            "server-action",
            user?._id,
          );
          ...
          sails.config.customLogger.log(
            "error",
            `Error sending forgot password email to user ${user._id}: ${emailError.message}`,
            null,
            "server-action",
            user?._id,
          );
      ...
      sails.config.customLogger.log(
        "error",
        `Unexpected error in forgotPassword: ${error?.message || error}`,
        null,
        "server-action",
        null,
      );
  ...
  async resetPassword(req, res) {
    ...
    if (!req.body.token) {
      sails.config.customLogger.log(
        "warn",
        "resetPassword attempted without token.",
        null,
        "message",
        null,
      );
    ...
    if (!passwordFormat.test(req.body.password)) {
      sails.config.customLogger.log(
        "warn",
        "resetPassword attempted with weak password.",
        null,
        "message",
        null,
      );
    ...
    try {
      ...
      await User.updateOne({ resetPasswordToken: token }).set({
        password,
        resetPasswordToken: "",
      });
      sails.config.customLogger.log(
        "info",
        `Password reset processed for user ${user?._id || user?.id}.`,
        null,
        "server-action",
        user?._id || user?.id,
      );
      ...
      sails.config.customLogger.log(
        "error",
        `Error in resetPassword: ${err.message}`,
        null,
        "server-action",
        null,
      );
  ...
  async loginLocal(req, res) {
    const { error: headersErrors, value: headers } = headersSchema.validate(
      req.headers,
      { abortEarly: false },
    );
    if (headersErrors) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Invalid request headers" });
    }
    ...
    if (!isLoginLocalAllowed) {
      ...
      return res
        .status(400)
        .json({ message: sails._t(locale, "password login is disabled") });
    }
    ...
    passport.authenticate("local", async (err, user, info = {}) => {
      if (err) {
        if (err?.message === "User is not approved") {
          sails.config.customLogger.log(
            "error",
            "User not approved in loginLocal.",
            null,
            "message",
            user?.id,
          );
          return res
            .status(403)
            .json({ message: sails._t(locale, "not approved") });
        }
        sails.config.customLogger.log(
          "error",
          `LoginLocal error: ${err?.message || "Unknown error"}`,
          null,
          "server-action",
          null,
        );
        return res.status(500).json({
          message:
            info.message || err?.message || sails._t(locale, "server error"),
        });
      }
      if (!user) {
        sails.config.customLogger.log(
          "warn",
          "LoginLocal failed: user not found.",
          null,
          "message",
          null,
        );
      ...
      try {
        ...
        sails.config.customLogger.log(
          "verbose",
          `User ${user.id} updated lastLoginType to local.`,
          null,
          "server-action",
          user?.id,
        );
        ...
        sails.config.customLogger.log(
          "error",
          `Error updating user ${user?.id} login type: ${error.message}`,
          null,
          "server-action",
          user?.id,
        );
      ...
      if (
        ...
        if (user.smsVerificationCode) {
          try {
            ...
            sails.config.customLogger.log(
              "error",
              `Error verifying smsVerificationCode for user ${user.id}: ${error?.message || error}`,
              null,
              "server-action",
              user?.id,
            );
        ...
        try {
          ...
          sails.config.customLogger.log(
            "info",
            `SMS sent to user ${user.id} with verification code.`,
            null,
            "server-action",
            user.id,
          );
          ...
          sails.config.customLogger.log(
            "error",
            `Error sending SMS to user ${user.id}: ${err.message}`,
            null,
            "server-action",
            user.id,
          );
        ...
        return res
          .status(200)
          .json({ localLoginToken, user: user.id, role: user.role });
      } else {
        if (user.smsVerificationCode) {
        ...
        req.logIn(user, function (err) {
          if (err) {
            sails.config.customLogger.log(
              "error",
              `Error logging in user ${user.id}: ${err.message}`,
              null,
              "server-action",
              user?.id,
            );
      ...
      if (err) {
        sails.config.customLogger.log(
          "error",
          `Error with LOGIN in loginLocal: ${err.message || "Unknown error"}`,
          null,
          "server-action",
          null,
        );
  ...
  loginSms(req, res) {
    ...
    passport.authenticate("sms", async (err, user, info = {}) => {
      if (err) {
        ...
        sails.config.customLogger.log(
          "error",
          `loginSms error: ${err?.message || "Unknown error"}`,
          null,
          "server-action",
          user?.id,
        );
      ...
      if (!user) {
        sails.config.customLogger.log(
          "warn",
          "loginSms failed: user not found.",
          null,
          "message",
          null,
        );
      ...
      try {
        ...
        sails.config.customLogger.log(
          "info",
          `User ${user.id} smsVerificationCode cleared.`,
          null,
          "server-action",
          user?.id,
        );
        ...
        sails.config.customLogger.log(
          "error",
          `Error updating user ${user.id} in loginSms: ${updateError.message}`,
          null,
          "server-action",
          user?.id,
        );
      ...
      sails.config.customLogger.log(
        "verbose",
        `User ${user.id} logged in via SMS.`,
        null,
        "message",
        user?.id,
      );
      ...
      if (err) {
        sails.config.customLogger.log(
          "error",
          `Error with LOGIN in loginSms: ${err?.message || "Unknown error"}`,
          null,
          "server-action",
          null,
        );
  ...
  login2FA(req, res) {
    ...
    passport.authenticate("2FA", (err, user, info = {}) => {
      if (err) {
        ...
        sails.config.customLogger.log(
          "error",
          `login2FA error: ${err?.message || "Unknown error"}`,
          null,
          "server-action",
          user?.id,
        );
        return res
          .status(500)
          .json({ message: info.message || err?.message || "Server Error" });
      }
      if (!user) {
        sails.config.customLogger.log(
          "warn",
          "login2FA failed: user not found.",
          null,
          "message",
          null,
        );
      ...
      req.logIn(user, function (err) {
        if (err) {
          sails.config.customLogger.log(
            "error",
            `Error logging in user ${user.id} in login2FA: ${err?.message}`,
            null,
            "server-action",
            user?.id,
          );
        ...
        sails.config.customLogger.log(
          "info",
          `User ${user.id} logged in via 2FA.`,
          null,
          "message",
          user?.id,
        );
      ...
      if (err) {
        sails.config.customLogger.log(
          "error",
          `Error with LOGIN in login2FA: ${err?.message || "Unknown error"}`,
          null,
          "server-action",
          null,
        );
  ...
  refreshToken: async function (req, res) {
    ...
    try {
      ...
      if (!user || user.status !== "approved") {
        sails.config.customLogger.log(
          "warn",
          `Refresh token attempt for invalid or unapproved user: ${decoded.id}`,
          null,
          "message",
          user?.id,
        );
      ...
      sails.config.customLogger.log(
        "info",
        `Refresh token successful for user ${user.id}.`,
        null,
        "server-action",
        user?.id,
      );
  ...
  verifyRefreshToken: async function (req, res) {
    ...
    if (!refreshToken) {
      sails.config.customLogger.log(
        "warn",
        "Refresh token missing in verifyRefreshToken.",
        null,
        "message",
        null,
      );
  ...
  logout(req, res) {
    const performLogout = () => {
      req.logout((err) => {
        if (err) {
          sails.config.customLogger.log(
            "error",
            `Error during req.logout: ${err.message}`,
            null,
            "server-action",
            req.user?.id,
          );
    ...
    if (
      ...
      try {
        ...
        sails.config.customLogger.log(
          "error",
          `Error logging out from SAML: ${error.message}`,
          null,
          "server-action",
          req.user?.id,
        );
  ...
  async getCurrentUser(req, res) {
    ...
    if (!req.user && !tokenValue) {
      sails.config.customLogger.log(
        "warn",
        "getCurrentUser: No token or user provided.",
        null,
        "message",
        null,
      );
    ...
    if (tokenValue) {
      jwt.verify(
        ...
        async (err, decoded) => {
          if (err) {
            sails.config.customLogger.log(
              "error",
              `JWT verification error in getCurrentUser: ${err?.message || err}`,
              null,
              "server-action",
              req.user?.id,
            );
          ...
          if (decoded.singleFactor) {
            sails.config.customLogger.log(
              "warn",
              "Token indicates single factor, unauthorized.",
              null,
              "message",
              req.user?.id,
            );
          ...
          try {
            if (version) {
              ...
              sails.config.customLogger.log(
                "info",
                `Updated doctorClientVersion for user ${decoded.id} to ${version}.`,
                null,
                "server-action",
                decoded.id,
              );
              ...
              sails.config.customLogger.log(
                "info",
                `Set doctorClientVersion to invalid for user ${decoded.id}.`,
                null,
                "server-action",
                decoded.id,
              );
            ...
            if (user.role === "doctor" && !user.doctorClientVersion) {
              sails.config.customLogger.log(
                "warn",
                `Doctor user ${user.id} has no valid client version.`,
                null,
                "message",
                user.id,
              );
            ...
            if (!req.user) {
              req.logIn(user, function (err) {
                if (err) {
                  sails.config.customLogger.log(
                    "error",
                    `Error logging in user ${user.id} in getCurrentUser: ${err?.message}`,
                    null,
                    "server-action",
                    user?.id,
                  );
                ...
                sails.config.customLogger.log(
                  "info",
                  `User ${user.id} logged in via token in getCurrentUser.`,
                  null,
                  "message",
                  user?.id,
                );
      ...
      sails.config.customLogger.log(
        "info",
        `getCurrentUser: Returning current user ${user.id}.`,
        null,
        "user-action",
        user?.id,
      );
  ...
  loginOpenId(req, res, next) {
    if (req.query.redirectUrl) {
      ...
      sails.config.customLogger.log(
        "info",
        `Stored redirectUrl in session: ${req.query.redirectUrl}`,
        null,
        "message",
        req.user?.id,
      );
    ...
    if (role === sails.config.globals.ROLE_DOCTOR) {
      sails.config.customLogger.log(
        "info",
        "Initiating OpenID Connect authentication for doctor.",
        null,
        "message",
        req.user?.id,
      );
    ...
    if (role === sails.config.globals.ROLE_ADMIN) {
      sails.config.customLogger.log(
        "info",
        "Initiating OpenID Connect authentication for admin.",
        null,
        "message",
        req.user?.id,
      );
    ...
    if (role === sails.config.globals.ROLE_NURSE) {
      sails.config.customLogger.log(
        "info",
        "Initiating OpenID Connect authentication for nurse.",
        null,
        "message",
        req.user?.id,
      );
    ...
    sails.config.customLogger.log(
      "warn",
      "loginOpenId: No valid role specified.",
      null,
      "message",
      req.user?.id,
    );
    return res
      .status(400)
      .json({ message: "Invalid role for OpenID Connect authentication." });
  },
        ...
        passport.authenticate(
          ...
          async (err, user, info = {}) => {
            if (err) {
              sails.config.customLogger.log(
                "error",
                `Error authenticating for admin: ${err?.message || err}`,
                null,
                "server-action",
                user?.id,
              );
            ...
            if (!user) {
              sails.config.customLogger.log(
                "warn",
                "Admin authentication did not return a user.",
                null,
                "message",
                null,
              );
            ...
            if (user.role === sails.config.globals.ROLE_ADMIN) {
              if (process.env.NODE_ENV === "development") {
                sails.config.customLogger.log(
                  "info",
                  `Admin user ${user.id} authenticated; redirecting to development URL.`,
                  null,
                  "server-action",
                  user?.id,
                );
                ...
                sails.config.customLogger.log(
                  "info",
                  `Admin user ${user.id} authenticated; redirecting to production URL.`,
                  null,
                  "server-action",
                  user?.id,
                );
          ...
          if (err) {
            sails.config.customLogger.log(
              "error",
              `Error in admin callback authentication: ${err?.message || err}`,
              null,
              "server-action",
              req.user?.id,
            );
        ...
        passport.authenticate(
          ...
          async (err, user, info = {}) => {
            if (err) {
              sails.config.customLogger.log(
                "error",
                `Error authenticating for nurse: ${err?.message || err}`,
                null,
                "server-action",
                user?.id,
              );
            ...
            if (!user) {
              sails.config.customLogger.log(
                "warn",
                "Nurse authentication did not return a user.",
                null,
                "message",
                req.user?.id,
              );
            ...
            if (
              ...
              if (process.env.NODE_ENV === "development") {
                sails.config.customLogger.log(
                  "info",
                  `Nurse user ${user.id} authenticated; redirecting to development URL.`,
                  null,
                  "server-action",
                  user.id,
                );
                ...
                sails.config.customLogger.log(
                  "info",
                  `Nurse user ${user.id} authenticated; redirecting to production URL.`,
                  null,
                  "server-action",
                  user.id,
                );
          ...
          if (err) {
            sails.config.customLogger.log(
              "error",
              `Error in nurse callback authentication: ${err?.message || err}`,
              null,
              "server-action",
              req.user?.id,
            );
        ...
        passport.authenticate(
          ...
          async (err, user, info = {}) => {
            if (err) {
              sails.config.customLogger.log(
                "error",
                `Error authenticating for doctor: ${err?.message || err}`,
                null,
                "server-action",
                user?.id,
              );
            ...
            if (!user) {
              sails.config.customLogger.log(
                "warn",
                "Doctor authentication did not return a user.",
                null,
                "message",
                req.user?.id,
              );
            ...
            try {
              ...
              sails.config.customLogger.log(
                "verbose",
                `User ${user.id} lastLoginType updated to 'openidconnect'.`,
                null,
                "server-action",
                user.id,
              );
            ...
            if (
              ...
              if (redirectUrl) {
                ...
                sails.config.customLogger.log(
                  "info",
                  `Using stored redirectUrl: ${redirectUrl}`,
                  null,
                  "server-action",
                  user?.id,
                );
              ...
              if (process.env.NODE_ENV === "development") {
                sails.config.customLogger.log(
                  "info",
                  `Doctor user ${user.id} authenticated; redirecting to development URL.`,
                  null,
                  "server-action",
                  user?.id,
                );
                ...
                sails.config.customLogger.log(
                  "info",
                  `Doctor user ${user.id} authenticated; redirecting to production URL.`,
                  null,
                  "server-action",
                  user?.id,
                );
          ...
          if (err) {
            sails.config.customLogger.log(
              "error",
              `Error in doctor callback authentication: ${err?.message || err}`,
              null,
              "server-action",
              req.user?.id,
            );
  ...
  getConfig(req, res) {
    ...
    sails.config.customLogger.log(
      "info",
      `getConfig called from IP: ${req.ip}`,
      null,
      "user-action",
      req?.user?.id,
    );
    ...
    res.json({
      ...
      skipConsultationPlanPage:
        sails.config.globals.SKIP_CONSULTATION_PLAN_PAGE,
      doctorLanguages,
      patientLanguages,
  ...
  externalAuth(req, res) {
    ...
    if (!token) {
      sails.config.customLogger.log(
        "warn",
        "externalAuth: Missing token in query.",
        null,
        "message",
        null,
      );
    ...
    jwt.verify(
      ...
      async (err, decoded) => {
        if (err) {
          sails.config.customLogger.log(
            "error",
            `externalAuth: JWT verification error: ${err?.message || err}`,
            null,
            "server-action",
            decoded?.id,
          );
        ...
        if (!decoded.timestamp) {
          sails.config.customLogger.log(
            "warn",
            "externalAuth: Token missing timestamp.",
            null,
            "message",
            decoded?.id,
          );
        ...
        try {
          ...
          if (now - tokenTimestamp > FIVE_MIN) {
            sails.config.customLogger.log(
              "warn",
              "externalAuth: Token timestamp is older than 5 minutes.",
              null,
              "message",
              decoded?.id,
            );
          ...
          sails.config.customLogger.log(
            "error",
            `externalAuth: Error processing token timestamp: ${error.message}`,
            null,
            "server-action",
            decoded?.id,
          );
        ...
        if (!decoded.email) {
          sails.config.customLogger.log(
            "warn",
            "externalAuth: Token missing email.",
            null,
            "message",
            decoded?.id,
          );
        ...
        try {
          ...
          if (!user) {
            ...
            sails.config.customLogger.log(
              "info",
              `externalAuth: Created new doctor user with id ${user.id}.`,
              null,
              "server-action",
              user.id,
            );
            ...
            sails.config.customLogger.log(
              "info",
              `externalAuth: Found existing doctor user with id ${user.id}.`,
              null,
              "message",
              user.id,
            );
          ...
          sails.config.customLogger.log(
            "info",
            `externalAuth: User ${user.id} authenticated successfully.`,
            null,
            "server-action",
            user.id,
          );
          ...
          sails.config.customLogger.log(
            "error",
            `externalAuth: Error processing external authentication: ${error.message}`,
            null,
            "server-action",
            null,
          );
